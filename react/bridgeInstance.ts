import type { Bridge, BridgeStore, ExtractStore, PrimitiveObject } from '../types';
...
export class BridgeInstance<T extends BridgeStore<T extends Bridge ? T : any>> {
  ...
  public store: Omit<T, 'setState'> = mockStore() as unknown as Omit<T, 'setState'>;

  public addEventListener<EventName>(eventName: EventName, listener: (args: any) => void) {
  ...
  private _createNativeMethod(methodName: string) {
    ...
    return (...args: unknown[]) => {
      console.log(`===== bridgeInstance // _createNativeMethod // evaluate // ${methodName} =====`);
  ...
  public _hydrate(bridgeMethods: string[], nativeInitialState: PrimitiveObject = {}) {
    ...
    const initialBridgeMethodNames = initialMethods.map(([methodName]) => methodName);
